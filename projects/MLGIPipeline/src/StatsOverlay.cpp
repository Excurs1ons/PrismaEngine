...
StatsOverlay::StatsOverlay(const ApplicationSpecification& spec, Graphic::PathTracingPipeline* pipeline,
                           bool enableNEE, bool usePrimitiveSphere, SampleSource* sampleSource,
...
void StatsOverlay::Update(Timestep ts) {
    ...
    if (m_overlayRefreshTimer >= 1.0f) {
        ...
        if (m_mlgiSystem) {
            auto mlgi = m_mlgiSystem->GetMetrics();
            if (mlgi.enabled) {
                m_mlgiMetricsStr = std::format(
                    "MLGI: ON  probes={}  rays/probe={}  temporalFrame={}  nonZeroCoeffs={}/{}",
                    mlgi.probeCount, mlgi.raysPerProbe,
                    mlgi.temporalFrameCount, mlgi.nonZeroCoeffCount, mlgi.totalCoeffCount);
                m_mlgiColor = {0.2f, 1.0f, 0.2f, 1.0f};
                m_mlgiFallbackStr.clear();
            } else {
                m_mlgiMetricsStr = "MLGI: DISABLED";
                m_mlgiColor = {0.6f, 0.6f, 0.6f, 1.0f};
                if (!mlgi.fallbackReason.empty()) {
                    m_mlgiFallbackStr = "MLGI fallback: " + mlgi.fallbackReason;
                } else {
                    m_mlgiFallbackStr.clear();
                }
            }
        } else {
            m_mlgiMetricsStr = "MLGI: N/A";
            m_mlgiColor = {0.6f, 0.6f, 0.6f, 1.0f};
            m_mlgiFallbackStr.clear();
        }
    ...
    Renderer2D::DrawString("MLGIPipeline",
    ...
    if (m_sampleSource && m_sampleSource->IsGenerated()) {
        std::string sampleInfo = std::format("Samples: {} {} (seed={})",
            m_sampleSource->GetCount(),
            (m_sampleSource->GetMode() == SampleMode::Halton) ? "Halton" : "Hammersley",
            m_sampleSource->GetSeed());
        Renderer2D::DrawString(sampleInfo, {30.0f, 205.0f}, 1.5f, {0.5f, 0.7f, 1.0f, 1.0f});
    }

    if (!m_mlgiMetricsStr.empty()) {
        Renderer2D::DrawString(m_mlgiMetricsStr, {30.0f, 235.0f}, 1.5f, m_mlgiColor);
    }
    if (!m_mlgiFallbackStr.empty()) {
        Renderer2D::DrawString(m_mlgiFallbackStr, {30.0f, 255.0f}, 1.5f, {1.0f, 0.6f, 0.2f, 1.0f});
    }
}
