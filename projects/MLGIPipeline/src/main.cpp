/**
 * MLGIPipeline — PrismaEngine MLGI 管线模板
 *
 * 演示 PrismaEngine 的 MLGI 管线能力：
 *   - Compute-shader 路径追踪（Cornell Box 场景）
 *   - 五面立方体：左红右绿，顶部白色面光源
 *
 * 编译: cmake --build build --build --target MLGIPipeline --config Debug
 * 运行: build/bin/Debug/MLGIPipeline
 */
...
int main(int argc, char* argv[]) {
    ...
    auto app = std::make_unique<Prisma::MLGIPipelineApp>();

    if (spec.Headless) {
        std::cout << "MLGIPipeline headless mode: frames=" << spec.HeadlessFrames
    ...
    if (result != 0) {
        std::cerr << "MLGIPipeline exited with error: " << result << std::endl;
    }
