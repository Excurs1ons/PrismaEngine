...
public class World : IDisposable
{
    ...
    public void Step(float dt)
    {
        Input.UpdateState(); 
        UpdateTime(dt);
        PrismaEngine.Time._deltaTime = this.Time.DeltaTime;
        ...
        finally { PrismaEngine.Time._isParallelStep = false; }
    ...
    public string SerializeScene()
    {
        ...
        foreach (var node in _nodes)
        {
            ...
            while (s != null)
            {
                ...
                s.OnSerialize(writer); // 调用 Source Generator 生成的代码 / Call code generated by Source Generator
                writer.WriteEndObject();
                writer.WriteEndObject();
                s = s._nextScript;
            }
