...
ApplicationSpecification Template2DApp::LoadSpecification(const std::string& filePath) {
    ...
    std::vector<std::string> potentialPaths = {
        ...
        "../projects/Template2D/assets/project.json" // 相对于 build/bin 运行时的源码位置
    };
    ...
    if (!file.is_open()) {
        LOG_INFO("Template2D", "未找到项目配置文件，使用默认设置");
        return spec;
    }
    ...
    try {
        ...
        if (data.contains("window")) {
            ...
            LOG_INFO("Template2D", "成功加载项目配置: {0}", filePath);
            LOG_INFO("Template2D", "  - 应用名称: {0}", spec.Name);
            LOG_INFO("Template2D", "  - 入口场景: {0}", spec.EntryScene);
            LOG_INFO("Template2D", "  - 窗口尺寸: {0}x{1}", spec.Width, spec.Height);
            LOG_INFO("Template2D", "  - 全屏: {0}", spec.Fullscreen ? "是" : "否");
            LOG_INFO("Template2D", "  - 可调大小: {0}", spec.Resizable ? "是" : "否");
            LOG_INFO("Template2D", "  - VSync 模式: {0}", vsyncMode);
            LOG_INFO("Template2D", "  - 最大 FPS: {0}", spec.MaxFPS);
        }
    } catch (const std::exception& e) {
        LOG_ERROR("Template2D", "解析项目配置文件失败: {0}", e.what());
    }
...
int Template2DApp::OnInitialize() {
    LOG_INFO("Template2D", "正在初始化 2D 场景模板...");

    // 添加项目特定的资产搜索路径
    auto assetManager = Engine::Get().GetAssetManager();
    if (assetManager) {
        assetManager->AddSearchPath("assets");                          // 本地构建输出目录
        assetManager->AddSearchPath("projects/Template2D/assets");    // 原始项目目录
    }
    ...
    if (sceneToLoad.empty()) {
        ...
        LOG_INFO("Template2D", "入口场景未配置，尝试加载默认场景: {0}", sceneToLoad);
    }
    ...
    if (!loaded) {
        if (m_Spec.EntryScene.empty()) {
            LOG_ERROR("Template2D", "入口场景未配置且未找到默认场景 'scenes/default.json'，程序退出。");
            return -1;
        }
        
        LOG_INFO("Template2D", "使用硬编码场景数据");
        // 初始化测试场景中的精灵
        m_sprites = {
    ...
    if (renderSystem && renderSystem->GetDevice()) {
        ...
        LOG_INFO("Template2D", "GPU: {0}", m_gpuName);
    }

    LOG_INFO("Template2D", "2D 场景模板初始化完成 (800x600, {} 个精灵)", m_sprites.size());

    if (m_autoQuit) {
        LOG_INFO("Template2D", "检测到 --quit 参数，正在自动退出...");
        Close();
    }
...
void Template2DApp::OnRender() {
    ...
    for (float x = 0.0f; x <= winW; x += step) {
        float alpha = (static_cast<int>(x) % 100 == 0) ? 0.15f : 0.08f;
    ...
    for (float y = 0.0f; y <= winH; y += step) {
        float alpha = (static_cast<int>(y) % 100 == 0) ? 0.15f : 0.08f;
        Graphic::Renderer2D::DrawQuad(
            {winW * 0.5f, y},
            {winW, 2.0f},
            {1.0f, 1.0f, 1.0f, alpha}
    ...
    for (const auto& sprite : m_sprites) {
        ...
        Graphic::Renderer2D::DrawString(coordStr, {sx + 10.0f, sy + 10.0f}, 2.0f, invColor);
    }

    // ---- 绘制 FPS 和说明文字 ----
    // 左下角：场景信息
    std::string infoText = "Template2D - " + std::to_string(m_sprites.size()) + " sprites";
    Graphic::Renderer2D::DrawString(infoText, {30.0f, 30.0f}, 2.0f, {0.6f, 0.6f, 0.6f, 1.0f});

    // 右上角：三行纵向排列 (字号加大，间距拉开)
    // 第一行：渲染分辨率 + FPS
    ...
    float resWidth = Graphic::Renderer2D::GetStringWidth(resText, 3.0f);
    Graphic::Renderer2D::DrawString(resText, {winW - resWidth - 30.0f, winH - 50.0f}, 3.0f, {0.4f, 0.7f, 0.4f, 1.0f});

    // 第二行：GPU 信息
    if (!m_gpuName.empty()) {
        float gpuWidth = Graphic::Renderer2D::GetStringWidth(m_gpuName, 2.0f);
        Graphic::Renderer2D::DrawString(m_gpuName, {winW - gpuWidth - 30.0f, winH - 95.0f}, 2.0f, {0.5f, 0.5f, 0.5f, 1.0f});
    }

    // 第三行：DrawCall
    auto renderStats = Graphic::Renderer2D::GetStats();
    std::string dcText = "DC: " + std::to_string(renderStats.QuadCount);
    float dcWidth = Graphic::Renderer2D::GetStringWidth(dcText, 2.0f);
    Graphic::Renderer2D::DrawString(dcText, {winW - dcWidth - 30.0f, winH - 135.0f}, 2.0f, {0.5f, 0.7f, 0.5f, 1.0f});
...
void Template2DApp::OnEvent(Event& e) {
    ...
    dispatcher.Dispatch<KeyPressedEvent>([this](KeyPressedEvent& ev) {
        ...
        if (ev.GetKeyCode() == SDL_SCANCODE_F11 && !ev.IsRepeat()) {
            ...
            LOG_INFO("Template2D", "F11: 请求 {0}x{1}, SDL 实际 {2}x{3}", newW, newH, actualW, actualH);
            return true;
        }
        return false;
    });
    ...
    dispatcher.Dispatch<WindowResizeEvent>([this](WindowResizeEvent& ev) {
        ...
        LOG_INFO("Template2D", "收到 WindowResizeEvent: {0}x{1}", w, h);
        m_Spec.Width = w;
        m_Spec.Height = h;
        if (m_camera) {
            m_camera->SetProjection(0.0f, static_cast<float>(w), 0.0f, static_cast<float>(h));
            LOG_INFO("Template2D", "相机投影更新为: 0-{0} x 0-{1}", w, h);
        }
        return false;
    });
}

bool Template2DApp::LoadScene(const std::string& filePath) {
    ...
    if (!file.is_open()) {
        LOG_ERROR("Template2D", "无法打开场景文件: {0} (实际路径: {1})", filePath, actualPath);
        return false;
    }
    ...
    try {
        ...
        LOG_INFO("Template2D", "正在从 JSON 加载场景: {0}", sceneName);
        ...
        if (data.contains("camera") && m_camera) {
            ...
            if (cameraJson.contains("projection")) {
                ...
                LOG_INFO("Template2D", "加载相机投影: L={0}, R={1}, B={2}, T={3}", left, right, bottom, top);
            }
        }
        ...
        if (data.contains("settings")) {
            ...
            if (settingsJson.contains("clearColor") && settingsJson["clearColor"].is_array() && m_camera) {
                ...
                LOG_INFO("Template2D", "  - 背景颜色: ({0}, {1}, {2}, {3})", r, g, b, a);
            }
        }
        ...
        if (data.contains("sprites") && data["sprites"].is_array()) {
            ...
            LOG_INFO("Template2D", "  - 加载精灵数量: {0}", m_sprites.size());
            return true;
        }
    } catch (const std::exception& e) {
        LOG_ERROR("Template2D", "解析场景文件失败: {0}", e.what());
    }
