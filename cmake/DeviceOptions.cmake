...
# Native 模式：仅使用平台 SDK 原生 API，不依赖第三方库
# Windows: DirectX12 + XAudio2 + RawInput
# Android: Vulkan + AAudio + GameActivity
option(PRISMA_USE_NATIVE "Use platform native APIs only (no third-party dependencies)" OFF)

# ========== 平台默认配置 ==========

if(PRISMA_USE_NATIVE)
    # Native 模式：使用平台 SDK 原生 API
    if(WIN32)
    ...
    elseif(ANDROID)
        ...
        set(PRISMA_ENABLE_AUDIO_SDL3_DEFAULT OFF)
    elseif(APPLE)
        # Apple Native: Metal + CoreAudio + Cocoa
        ...
        # Linux Native: OpenGL/DRM + ALSA/PulseAudio + evdev
        set(PRISMA_ENABLE_RENDER_OPENGL_DEFAULT ON)
...
option(PRISMA_ENABLE_AUDIO_OPENSLES "Enable OpenSL ES (Android native)" OFF)
...
# 跨平台第三方库
option(PRISMA_ENABLE_AUDIO_OPENAL "Enable OpenAL (cross-platform)" ${PRISMA_ENABLE_AUDIO_OPENAL_DEFAULT})
...
option(PRISMA_ENABLE_INPUT_WIN32 "Enable Win32 Input (Windows native)" OFF)
option(PRISMA_ENABLE_INPUT_XINPUT "Enable XInput (Windows gamepad)" OFF)
...
# 跨平台第三方库
option(PRISMA_ENABLE_INPUT_SDL3 "Enable SDL3 Input (cross-platform)" OFF)
...
# Android 平台检查
if(NOT ANDROID)
    if(PRISMA_ENABLE_AUDIO_AAUDIO OR PRISMA_ENABLE_AUDIO_OPENSLES)
        message(WARNING "Android audio APIs are only supported on Android. Disabling...")
        set(PRISMA_ENABLE_AUDIO_AAUDIO OFF CACHE BOOL "" FORCE)
        set(PRISMA_ENABLE_AUDIO_OPENSLES OFF CACHE BOOL "" FORCE)
    endif()
...
# Linux 平台检查 (Linux 检查需要更复杂，因为 CMAKE_SYSTEM_NAME 在 Linux 上是 "Linux")
...
if(PRISMA_ENABLE_AUDIO_OPENSLES)
    set(HAS_AUDIO_DEVICE ON)
    list(APPEND PRISMA_AUDIO_LIST "OpenSL_ES")
endif()
if(PRISMA_ENABLE_AUDIO_COREAUDIO)
    set(HAS_AUDIO_DEVICE ON)
...
if(PRISMA_ENABLE_AUDIO_OPENAL)
    set(HAS_AUDIO_DEVICE ON)
    list(APPEND PRISMA_AUDIO_LIST "OpenAL")
endif()
...
# Native 模式标记
if(PRISMA_USE_NATIVE)
    add_definitions(-DPRISMA_USE_NATIVE=1)
endif()
...
if(PRISMA_ENABLE_AUDIO_OPENSLES)
    add_definitions(-DPRISMA_ENABLE_AUDIO_OPENSLES=1)
endif()
if(PRISMA_ENABLE_AUDIO_COREAUDIO)
...
message(STATUS "Mode: ${PRISMA_USE_NATIVE}")
