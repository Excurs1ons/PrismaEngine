// ============================================================
// npr_lit.vert - PrismaEngine NPR 前向渲染顶点着色器
//
// 输入: position, color, uv, normal, texCoord, tangent
// 输出: 世界空间位置, 法线, UV, 切线, 视角方向, 颜色
// Push Constants: 世界矩阵
...
void main() {
    ...
    if (length(inTangent.xyz) > 0.001) {
        v_Tangent = normalize(normalMatrix * inTangent.xyz);
    } else {
        v_Tangent = vec3(0.0);
    }
