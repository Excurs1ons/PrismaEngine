...
set "cmake_args=-DCMAKE_TOOLCHAIN_FILE=%ANDROID_NDK_HOME%\build\cmake\android.toolchain.cmake -DANDROID_ABI=%abi% -DANDROID_PLATFORM=%ANDROID_PLATFORM% -DANDROID_STL=%ANDROID_STL% -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DCMAKE_INSTALL_PREFIX=..\..\build\install\%abi% -DPRISMA_ENABLE_AUDIO_XAUDIO2=OFF -DPRISMA_ENABLE_AUDIO_SDL3=ON -DPRISMA_ENABLE_RENDER_DX12=OFF -DPRISMA_ENABLE_RENDER_VULKAN=ON -DPRISMA_BUILD_EDITOR=OFF -G Ninja"
