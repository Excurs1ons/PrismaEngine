...
/// 
/// </summary>
/// <param name="hr">内部为long,表示执行结果</param>
// C++
inline void ThrowIfFailed(HRESULT hr)
{
    if (FAILED(hr))
    {
        LOG_FATAL("DirectX", "FAILED: {0}", HrToString(hr));
...
inline HRESULT ReadDataFromFile(LPCWSTR filename, uint8_t** data, UINT* size)
{
...
inline HRESULT ReadDataFromDDSFile(LPCWSTR filename, uint8_t** data, UINT* offset, UINT* size)
{
