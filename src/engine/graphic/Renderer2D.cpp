...
void Renderer2D::Initialize() {
    ...
    if (device && device->GetResourceFactory()) {
        ...
        auto vbo = resourceManager->CreateBuffer(vDesc);

        BufferDesc iDesc;
        iDesc.type = BufferType::Index;
        iDesc.size = indices.size() * sizeof(uint32_t);
        iDesc.initialData = indices.data();
        auto ibo = resourceManager->CreateBuffer(iDesc);

        SubMeshBuffer subMesh;
        subMesh.vertexBuffer = vbo;
        subMesh.indexBuffer = ibo;
...
void Renderer2D::EndScene() {
    ...
    if (s_Data && s_Data->Stats.QuadCount > 0) {
        ...
        if (!firstRenderReported) {
            LOG_INFO("Renderer2D", "首次渲染成功触发，本帧渲染了 %u 个 Quad。", s_Data->Stats.QuadCount);
        ...
        if (++frameCount % 100 == 0) {
            LOG_DEBUG("Renderer2D", "渲染统计: %u 个 Quad。", s_Data->Stats.QuadCount);
        }
    }
}
...
void Renderer2D::DrawQuad(const Matrix4& transform, const Prisma::Color& /*color*/) {
    ...
    Renderer::Submit(s_Data->QuadMesh.get(), s_Data->DefaultMaterial.get(), transform);
...
void Renderer2D::DrawQuad(const Matrix4& transform, const std::shared_ptr<ITexture>& texture, const Prisma::Color& /*tintColor*/) {
    ...
    Renderer::Submit(s_Data->QuadMesh.get(), material.get(), transform);
    s_Data->Stats.QuadCount++;
}
