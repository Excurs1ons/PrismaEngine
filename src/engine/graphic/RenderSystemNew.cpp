...
bool RenderSystem::Initialize(const RenderSystemDesc& desc) {
    ...
    LOG_INFO("Render", "渲染后端类型: {}", static_cast<int>(desc.backendType));

// 检查编译时启用的渲染后端
#if defined(PRISMA_ENABLE_RENDER_DX12)
    LOG_INFO("Render", "编译时启用的后端: DirectX12");
#endif
#if defined(PRISMA_ENABLE_RENDER_VULKAN)
    LOG_INFO("Render", "编译时启用的后端: Vulkan");
#endif
#if defined(PRISMA_ENABLE_RENDER_OPENGL)
    LOG_INFO("Render", "编译时启用的后端: OpenGL");
#endif
...
RenderContext RenderSystem::GetRenderContext() const {
    ...
    // 添加场景数据
    auto sceneManager = ::PrismaEngine::SceneManager::GetInstance();
    if (sceneManager) {
