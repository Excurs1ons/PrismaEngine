...
const ShaderReflection::Resource* VulkanShader::FindResource(const std::string& name) const {
...
const ShaderReflection::Resource* VulkanShader::FindResourceByBindPoint(uint32_t bindPoint, uint32_t space) const {
    for (const auto& resource : m_reflection.resources) {
...
const ShaderReflection::ConstantBuffer* VulkanShader::FindConstantBuffer(const std::string& name) const {
...
const ShaderReflection::InputParameter& VulkanShader::GetInputParameter(uint32_t index) const {
...
const ShaderReflection::OutputParameter& VulkanShader::GetOutputParameter(uint32_t index) const {
...
bool VulkanShader::DebugSaveToFile(const std::string& filename,
    ...
    // 保存SPIR-V字节码
    file.write(reinterpret_cast<const char*>(m_spirv.data()), m_spirv.size() * sizeof(uint32_t));
...
bool VulkanShader::CreateShaderModule() {
    ...
    if (result != VK_SUCCESS) {
        m_compileLog = "Failed to create Vulkan shader module: " + std::to_string(result);
        return false;
    }
