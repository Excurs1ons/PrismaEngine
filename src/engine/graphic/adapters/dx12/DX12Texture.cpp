...
ResourceId DX12Texture::GetId() const {
    return static_cast<ResourceId>(reinterpret_cast<uintptr_t>(m_resource.Get()));
}

const std::string& DX12Texture::GetName() const {
    return m_name;
}
...
uint64_t DX12Texture::GetSize() const {
    if (!m_resource) {
        return 0;
    }
...
void DX12Texture::AddRef() {
    ++m_refCount;
}

uint32_t DX12Texture::Release() {
    if (--m_refCount == 0) {
        delete this;
        return 0;
    }
...
uint32_t DX12Texture::GetRefCount() const {
    return m_refCount;
}

const std::string& DX12Texture::GetDebugTag() const {
    return m_debugTag;
}

void DX12Texture::SetDebugTag(const std::string& tag) {
    m_debugTag = tag;
}

uint64_t DX12Texture::GetCreationTimestamp() const {
    return m_creationTimestamp;
}

uint64_t DX12Texture::GetLastAccessTimestamp() const {
    return m_lastAccessTimestamp;
}

void DX12Texture::MarkDirty() {
    m_isDirty = true;
}

bool DX12Texture::IsDirty() const {
    return m_isDirty;
}

void DX12Texture::ClearDirty() {
    m_isDirty = false;
}
