...
struct SkylineNode {
    ...
    SkylineNode(int x = 0, int y = 0, int width = 0) : x(x), y(y), width(width) {}
};
...
public:
    SkylinePacker(int atlasWidth, int atlasHeight, int padding)
        : m_atlasWidth(atlasWidth), m_atlasHeight(atlasHeight), m_padding(padding) {
...
public:
    explicit TextureAtlasImpl(const AtlasConfig& config) : m_config(config) {
    ...
    bool addTexture(const std::string& texturePath, const std::string& name) override {
        ...
        m_sourceTextures.emplace(
            name, SourceTexture(asset.GetData(), asset.GetWidth(), asset.GetHeight(), asset.GetChannels(), name));
    ...
    bool build() override {
        ...
        TextureAtlasBuilder* builder =
            TextureAtlasBuilderFactory::create(TextureAtlasBuilder::PackingAlgorithm::SKYLINE).release();
    ...
    bool hasRegion(const std::string& name) const override { return m_regions.find(name) != m_regions.end(); }
    ...
    size_t getRegionCount() const override { return m_regions.size(); }
...
public:
    explicit BlockTextureManagerImpl(const AtlasConfig& config) : m_atlas(TextureAtlas::create(config)) {
    ...
    bool buildAtlas() override { return m_atlas->build(); }
    ...
    const TextureRegion* getFaceRegion(const std::string& blockName, FaceDirection direction) const override {
        ...
        switch (direction) {
            case FaceDirection::TOP:
                textureName = block.top;
                break;
            case FaceDirection::BOTTOM:
                textureName = block.bottom;
                break;
            case FaceDirection::NORTH:
                textureName = block.north;
                break;
            case FaceDirection::SOUTH:
                textureName = block.south;
                break;
            case FaceDirection::WEST:
                textureName = block.west;
                break;
            case FaceDirection::EAST:
                textureName = block.east;
                break;
    ...
    ITexture* getAtlasTexture() const override { return m_atlas->getAtlasTexture(); }

    bool hasBlock(const std::string& blockName) const override { return m_blocks.find(blockName) != m_blocks.end(); }

    size_t getBlockCount() const override { return m_blocks.size(); }

    static void registerDefaultBlocks(BlockTextureManager* manager) { manager->registerBlocks(getDefaultBlocks()); }
...
private:
    static const std::vector<BlockTexture> getDefaultBlocks() {
        return {
            ...
            BlockTexture("crafting_table_top",
                         "crafting_table_top",
...
public:
    TextureAtlasBuilderImpl(PackingAlgorithm algorithm) : m_algorithm(algorithm) {
        LOG_INFO("TextureAtlasBuilder",
                 "创建纹理图集构建器，算法: {}",
    ...
    void setPackingAlgorithm(PackingAlgorithm algorithm) override { m_algorithm = algorithm; }
    ...
    void clear() override { m_sourceTextures.clear(); }

    size_t getSourceTextureCount() const override { return m_sourceTextures.size(); }
...
private:
    ...
    void copyTextureToAtlas(std::vector<uint8_t>& atlasData,
                            uint32_t atlasWidth,
...
std::unique_ptr<TextureAtlasBuilder>
TextureAtlasBuilderFactory::create(TextureAtlasBuilder::PackingAlgorithm algorithm) {
