...
std::string Logger::FormatCallStack(const std::vector<StackFrame>& callStack) {
    ...
    for (size_t i = 0; i < callStack.size(); ++i) {
        ...
        oss << "  " << i << ". " << frame.function << " (" << filePath.filename().string() << ":" << frame.line
            << ")\n";
    }
...
void Logger::RotateLogFile() {
    ...
    for (int i = static_cast<int>(config_.maxFileCount) - 1; i > 0; --i) {
        std::string oldFile = std::format("{}/{}_{}{}", directory.string(), baseName, i, extension);
        std::string newFile = std::format("{}/{}_{}{}", directory.string(), baseName, i + 1, extension);
    ...
    // 重命名当前文件
    std::string backupFile = std::format("{}/{}_{}{}", directory.string(), baseName, 1, extension);
