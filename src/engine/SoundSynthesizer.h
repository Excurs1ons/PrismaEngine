...
public:
    SquareWaveGenerator(float sampleRate = 44100.0f) : WaveformGenerator(sampleRate), dutyCycle(0.5f) {}
...
public:
    TriangleWaveGenerator(float sampleRate = 44100.0f) : WaveformGenerator(sampleRate) {}
    void GenerateSamples(float* buffer, size_t sampleCount) override {
...
public:
    NoiseGenerator(float sampleRate = 44100.0f) : WaveformGenerator(sampleRate), lfsr(1), period(1) {}
...
public:
    SoundSynthesizer(float sampleRate)
        : pulse1(sampleRate), pulse2(sampleRate), triangle(sampleRate), noise(sampleRate) {
        // 设置FC典型的参数
    ...
    void NoteOn(int channel, float frequency) {
        switch (channel) {
            case 0:
                pulse1.SetFrequency(frequency);
                break;
            case 1:
                pulse2.SetFrequency(frequency);
                break;
            case 2:
