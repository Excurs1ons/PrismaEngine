...
PrismaMath::vec2 UIComponent::GetScreenPosition() const {
    ...
    // 计算锚点在父组件中的位置
    PrismaMath::vec2 anchorPos = parentPos + PrismaMath::vec2(parentSize.x * m_anchor.x, parentSize.y * m_anchor.y);

    // 减去枢轴偏移（使枢轴对齐到锚点）
    PrismaMath::vec2 pivotOffset = PrismaMath::vec2(m_size.x * m_pivot.x, m_size.y * m_pivot.y);
...
bool UIComponent::HitTest(const PrismaMath::vec2& point) const {
    ...
    return point.x >= screenPos.x && point.x <= screenPos.x + m_size.x && point.y >= screenPos.y &&
