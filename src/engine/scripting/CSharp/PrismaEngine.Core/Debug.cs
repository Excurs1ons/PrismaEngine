...
public ref struct PrismaLogInterpolatedStringHandler
{
    ...
    {
        // 增加预估空间，并多留一个字节给 null 终止符
        int initialSize = literalLength + formattedCount * 64;
        
        if (initialSize < MaxStackSize)
        {
            // 在 C# 11+ 中，可以通过这种方式在构造函数中处理 stackalloc (实际上是调用处分配)
            // 但为了兼容性，我们这里使用一个预留的字段或让调用者传递
            // 实际上 InterpolatedStringHandler 编译器生成的代码支持 stackalloc 分配缓冲区传进来
            // 这里我们先实现池化，但在构造函数里区分大小。
            _arrayFromPool = null;
            // 注意：stackalloc 只能在方法内。构造函数无法持有 stackalloc 的 span 除非它是 ref struct 且由外部传入。
    ...
    {
        // 检查是否支持 ISpanFormattable (包括 Vector2, Vector3, Color)
