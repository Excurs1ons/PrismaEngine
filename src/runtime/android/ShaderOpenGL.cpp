...
ShaderOpenGL *ShaderOpenGL::loadShader(
...
GLuint ShaderOpenGL::loadShader(GLenum shaderType, const std::string &shaderSource) {
...
void ShaderOpenGL::activate() const {
    glUseProgram(program_);
}

void ShaderOpenGL::deactivate() const {
    glUseProgram(0);
}

void ShaderOpenGL::drawModel(const Model &model) const {
    ...
    glVertexAttribPointer(
            ...
            GL_FALSE, // don't normalize
            sizeof(Vertex), // stride is Vertex bytes
            ((uint8_t *) model.getVertexData()) + offsetof(Vertex, uv) // offset Vector3 from the start
    );
...
void ShaderOpenGL::setProjectionMatrix(float *projectionMatrix) const {
