...
public:
    ...
    // 设置颜色
    void SetColor(const Prisma::Color& color) { m_color = color; m_dirty = true; }
    ...
    // 获取字体图集
    std::shared_ptr<Graphic::FontAtlas> GetFontAtlas() const { return m_fontAtlas; }

    // 获取顶点数据（用于渲染）
    const std::vector<Graphic::Vertex>& GetVertices() const { return m_vertices; }
...
private:
    ...
    std::shared_ptr<Graphic::FontAtlas> m_fontAtlas;
    bool m_dirty;

    // 渲染数据
    std::vector<Graphic::Vertex> m_vertices;
